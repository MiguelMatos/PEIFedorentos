...
public class CreateAdapterVisitor extends ASTVisitor {
	...
	private void CreateAdapter() {
		...
		classDec.setName(adapterAST.newSimpleName(typeName.getFullyQualifiedName()));
		
		//body
		for(MethodDeclaration oldMethodDeclaration : methodDeclarations) {
			MethodDeclaration newMethodDeclaration = adapterAST.newMethodDeclaration();
			
			newMethodDeclaration.setName(adapterAST.newSimpleName(oldMethodDeclaration.getName().getFullyQualifiedName()));
			newMethodDeclaration.modifiers().add("public");
			newMethodDeclaration.parameters().addAll(oldMethodDeclaration.parameters());
			...
			MethodInvocation methodInvocation = adapterAST.newMethodInvocation();
			SimpleName className = adapterAST.newSimpleName(typeName.getFullyQualifiedName());
			SimpleName methodName = adapterAST.newSimpleName(oldMethodDeclaration.getName().getFullyQualifiedName());
			...
			for (int a = 0; a < oldMethodDeclaration.parameters().size(); a++) {
				if (oldMethodDeclaration.parameters().get(a) instanceof SingleVariableDeclaration) {
					SingleVariableDeclaration dec = (SingleVariableDeclaration) oldMethodDeclaration.parameters().get(a);
					arguments.add(adapterAST.newSimpleName(dec.getName().getFullyQualifiedName()));
